...
async def test():
    ...
    async with ThingsBoardClient(tb_host, tb_user, tb_pass) as tb:
        ...
        df_weather = pd.DataFrame({'ghi': [943.9], 'poa': [943.9], 'air_temp': [30], 'wind_speed': [2]}, index=idx)

        # Force use_poa to False to see what Perez model does
        config.orientations[0].use_measured_poa = False
        res = compute_ac_power(config, df_weather, 'tb_station')
        print("Calculated Power with Perez (GHI):")
